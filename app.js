...
function loadToday(){
  ...
  db.collection("proteinTracker").doc(id).onSnapshot(s=>{
    ...
    USERS.forEach(u=>{
      ...
      el(`today${u}`).style.width=Math.min(100,p)+"%";
      el(`today${u}`).className="fill "+color(p);
...
function loadWeek(offset){
  ...
  for(let i=0;i<7;i++){
    ...
    const clickable=isThisWeek && d<=new Date();
    ...
    if(clickable){
      ...
      elDay.onclick=()=>{ activeDate=id; hideUndo(); loadWeek(0); };
    }
    ...
    setTimeout(()=>{
      db.collection("proteinTracker").doc(id).onSnapshot(s=>{
        ...
        USERS.forEach(u=>{
          const b=el(`w-${id}-${u}`);
          if(b) b.style.width=Math.min(100,(data[u]||0)/TARGET*100)+"%";
        });
      });
    },i*60);
  }
...
el("saveBtn").onclick=async()=>{
  ...
  setUndo({user:currentUser,date:activeDate,value:v});
};
...
function setUndo(e){
  ...
  el("undoBtn").style.display="inline";
...
el("undoBtn").onclick=async()=>{
  ...
  await db.collection("proteinTracker").doc(undoEntry.date)
    .set({[undoEntry.user]:firebase.firestore.FieldValue.increment(-undoEntry.value)},{merge:true});
  hideUndo();
};
